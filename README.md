...
The Second one is by entering the znet-environment:
...
To start a fully-featured set, you may run:

```
$ znet start --profiles=3cored,faucet,explorer,monitoring
```
...
with `znet remove` and only then you can start the new env.
...
- run `rm -rf ~/.cache/znet` to remove all the files created by `znet`
