...
func rootCmd(ctx context.Context, configF *infra.ConfigFactory, cmdF *CmdFactory) *cobra.Command {
	...
	rootCmd.PersistentFlags().StringVar(
		&configF.EnvName,
		"env",
		defaultString("ZNET_ENV", "znet"),
		"Name of the environment to run in",
	)
	rootCmd.PersistentFlags().StringVar(
		&configF.HomeDir,
		"home",
		defaultString("ZNET_HOME", must.String(os.UserCacheDir())+"/znet"),
...
func addRootDirFlag(cmd *cobra.Command, configF *infra.ConfigFactory) {
	cmd.Flags().StringVar(
		&configF.RootDir,
		"root-dir",
		defaultString("ZNET_ROOT_DIR", repoRoot()),
...
func addProfileFlag(cmd *cobra.Command, configF *infra.ConfigFactory) {
	cmd.Flags().StringSliceVar(
		&configF.Profiles,
		"profiles",
		defaultStrings("ZNET_PROFILES", apps.DefaultProfiles()),
...
func addTimeoutCommitFlag(cmd *cobra.Command, configF *infra.ConfigFactory) {
	defaultTimeoutCommitString := defaultString("ZNET_TIMEOUT_COMMIT", "0s")
...
func addCoredVersionFlag(cmd *cobra.Command, configF *infra.ConfigFactory) {
	cmd.Flags().StringVar(
		...
		"cored-version",
		defaultString("ZNET_CORED_VERSION", ""),
...
func addCoverageOutputFlag(cmd *cobra.Command, configF *infra.ConfigFactory) {
	cmd.Flags().StringVar(
		...
		"coverage-output",
		defaultString("ZNET_COVERAGE_OUTPUT",
...
func repoRoot() string {
	...
	// to detect znet repo root we go 3 levels up.
